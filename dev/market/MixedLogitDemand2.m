...
%% GENERAL DEMAND %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%       
        function [s, varargout] = shares(obj, p, varargin)
            ...
            if any(nonlinprice)
                ...
                obj.nlpart(obj.rc_sigma); % Calculates obj.mu. Only necessary once in simulating!
            end
            if nargin == 2 
...
%             if nargout > 2
%                 nopurch = 1 - obj.dummarket' * sum(share);
%                 varargout{2} = nopurch(obj.marketid, :);
%             end
        end
        ...
        function sh = shareJacobianAnal(obj, P)
            ...
            if any(nonlinprice) 
                ...
                for i = 1:size(si,2)
                    aS = ms(:, i);
%                     sh = sh + obj.quadw(i)*( diag(aS) - aS * si(:, i)' );
...
%% Init %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
        ...
        function initPeriods(obj)
            ...
            for t = 1:max(obj.marketid)
                ...
                newmarket.s = obj.s(newmarket.selection, 1);
                newmarket.p = obj.p(newmarket.selection, 1);
                newmarket.v = [];
                for k = 1:size(obj.x2, 2)
                    newmarket.v(:,:,k) = obj.v(newmarket.selection,:,k);
                end
                newmarket.init();
        ...
        function initSimulation(obj, varargin)
            if nargin > 1
                obj.selection = varargin{1}' * obj.dummarket;
                %NEW
                obj.isselected = obj.selection  > 0;
                if (sum(obj.dummarket) - obj.selection)*obj.isselected' > 0
                    warning('Partial select of some markets')
                end
            else
                obj.isselected = ones(1, size(obj.dummarket, 2));
            end
            if ~isempty(obj.beta) && ~isempty(obj.rc_sigma)
                % Create starting values for finddelta
                if isempty(obj.edelta) 
                    % as finddelta depends on selection and edelta is set,
                    % there is no need to redo calculation
                    if isempty(obj.s)
                ...
                for t = 1:max(obj.marketid)
                    obj.period{t}.d = obj.d(obj.period{t}.selection, 1);
                    obj.period{t}.alpha = obj.alpha;
                    obj.period{t}.rc_sigma = obj.rc_sigma;
                end
            else
    ...
    methods % (Access = private)     
        function newedelta = finddelta(obj, rc_sigma, edelta, tolerance)
            ...
            newedelta = zeros(size(edelta));
                for t = 1:max(obj.marketid)
                    if ~isempty(obj.isselected) && ~obj.isselected(t)
                        continue
                    end
                    newedelta(sel(:,t)) = obj.period{t}.finddelta(rc_sigma, ...
                        edelta(sel(:,t)), tolerance);
                end
%             obj.config.fp_iter = obj.config.fp_iter + i;
%             obj.config.fp_invoc = obj.config.fp_invoc + 1;
        end
        ...
        % Computes the mu or nonlinear part of utility
            ...
            for t = 1:max(obj.marketid)
                obj.period{t}.nlpart( rc_sigma );
            end
         end
        
        function [s,is] = sharecalc(obj, edelta)
            s = zeros(size(edelta));
            is = zeros(size(edelta, 1), obj.settings.nind);
            for t = 1:max(obj.marketid)
                if ~isempty(obj.isselected) && ~obj.isselected(t)
                    continue
                end
                selection = logical(obj.dummarket(:,t));
                [sm, ism] = obj.period{t}.sharecalc( edelta(selection) );
                s(selection) = sm;
                is(selection,:) = ism;
            end            
        end
